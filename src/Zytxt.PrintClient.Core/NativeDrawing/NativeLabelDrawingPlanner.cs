...
public sealed class NativeLabelDrawingPlanner
{
    public NativeLabelDrawingPlan CreatePlan(LabelRenderPlan labelPlan, LabelOffset? offset = null)
    {
        ...
        {
            Text(0m + offsetX, 0m + offsetY, 9.2m, 1.5m, labelPlan.IdentifierText, 4.2m, bold: true),
            Text(0m + offsetX, 1.5m + offsetY, 2m, 8.5m, "合格证", 4.6m, bold: false),
            QrCode(1.8m + offsetX, 1m + offsetY, 9m, labelPlan.QrPayload),
            Text(1.8m + offsetX, 9m + offsetY, 9m, 1.2m, "标签约0.20g", 3.6m, bold: false),
            Text(10.2m + offsetX, 0m + offsetY, 16.3m, 6.4m, labelPlan.ProductName, 4.2m, bold: false, maxLines: 4, ellipsis: true),
            ...
            Text(0m + offsetX, 14.6m + offsetY, 9m, 1.8m, labelPlan.SalesCode, 4.2m, bold: true)
        };
        ...
        if (!string.IsNullOrWhiteSpace(labelPlan.FooterText))
        {
            ...
            commands.Add(Text(0m + offsetX, footerY + offsetY, 24m, footerHeight, labelPlan.FooterText, 4.0m, bold: false));
        }

        commands.Add(Text(22.5m + offsetX, 15.6m + offsetY, 2.2m, 10m, labelPlan.IdentifierText, 4.2m, bold: true, rotationDegrees: 90m));
    ...
    private static void AddWeightCommands(List<NativeDrawCommand> commands, string text, decimal x, decimal y)
    {
        ...
        commands.Add(Text(x + 7.9m, y - 0.08m, 7.3m, 1.9m, value, 4.5m, bold: false));
    }
